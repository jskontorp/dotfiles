...
- **Database migrations**: schema-mutating migration commands are out of scope without approval. Generating an empty migration scaffold is fine; applying, rolling back, or autogenerating against a live DB is not. Same rule for raw `psql` / SQL that mutates schema or data outside a sandbox.
- **Destructive git**: `push --force` (lease check is no safer from this side of the conversation), `reset --hard`, and any other history-rewriting, ref-discarding, or working-tree-discarding operation are out of scope without approval.
