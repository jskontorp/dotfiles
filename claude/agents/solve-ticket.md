...
SCRIPTS="$HOME/code/personal/dotfiles/pi/agent/skills/solve-ticket/scripts"
```
...
```bash
REVIEW_CMD="claude -p" "$SCRIPTS/peer-review-spawn.sh" "$TICKET" "$WT" "$STATE_DIR" "$BASE"
RC=$?
```
...
1. **Commit** — invoke the `/commit` skill, stopping after step 3 (commits created). Its step 4 (push prompt + summary) and step 5 (PR description update) don't apply here — `create-pr` handles the push, and there's no PR yet to update.
2. **Draft PR** — invoke the `/create-pr` skill. Include `Closes TECH-123` in the body.
