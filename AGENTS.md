...
| `git/hooks/pre-commit`, `git/lib/secret-gate.sh`, `pi/agent/extensions/secret-read-gate.ts`, `claude/settings.json` deny block | `just check` | `just test` | Secret-path gate (JSK-35). The three pattern sets must move together — `git/lib/secret-gate.sh` is canonical; mirror in the `.ts` extension and Claude `Read`/`Edit`/`Write` deny entries. `test/check-secret-gate.sh` covers unit + integration. |
